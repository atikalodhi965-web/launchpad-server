...
const upload = multer({
  storage: multer.memoryStorage(), // keep file in memory (no temp files)
});
const PINATA_JWT = process.env.PINATA_JWT!;
const PINATA_GATEWAY = process.env.PINATA_GATEWAY;
console.log('PINATA_JWT loaded:', !!PINATA_JWT);
console.log('PINATA_GATEWAY loaded:', !!PINATA_GATEWAY);
/**
 * ------------------------------------------
...
//       console.log(`[Profile Upload] Request received. UserId provided: ${userId || 'none'}`);
...
//       const tempFileName = `profile-${userId || 'temp'}-${Date.now()}.${outputFormat}`;
...
//         symbol: 'PFP',
//         description: `Profile picture for user ${userId || 'anonymous'}`,
//         showName: false,
//       };
...
//       // 7) Upsert user in "users" table, setting profile_picture_url IF userId is provided
//       if (userId) {
...
//             profile_image_url: imageUrl, // Use the extracted image URL
//             created_at: new Date(),
//             updated_at: new Date(),
//           });
...
//         }
//       } else {
//         console.log('[Profile Upload] No userId provided, skipping database update');
//       }
...
profileImageRouter.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      // 1. Prepare form-data for Pinata
      const formData = new FormData();
      console.log(`[Profile Upload] Received file: ${req.file.originalname}, size: ${req.file.size} bytes, mimetype: ${req.file.mimetype}`);
      ...
      console.log(`[Profile Upload] Pinata response status: ${response.status}`);
      ...
      console.log(`[Profile Upload] Pinata response data: ${JSON.stringify(data)}`);
      // 3. Build image URL
      const imageUrl = `https://${PINATA_GATEWAY}/ipfs/${data.IpfsHash}`;
