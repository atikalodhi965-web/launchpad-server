...
export async function uploadFileToPinata(
  ...
  try {
    ...
    const gateway = (process.env.PINATA_GATEWAY && process.env.PINATA_GATEWAY !== 'undefined') ? process.env.PINATA_GATEWAY : 'ipfs.io';
    const finalUrl = `https://${gateway}/ipfs/${data.IpfsHash}`;
    console.log('[IPFS] Generated file URL:', finalUrl, 'using gateway:', gateway);
    return finalUrl;
  } finally {
 ...
 * @param metadataObject - The JSON object to upload
 * @returns - The IPFS URI for the uploaded JSON
 */
export async function uploadJsonToPinata(metadataObject: object): Promise<string> {
  const { default: fetch } = await import('node-fetch');
  ...
  const gateway = (process.env.PINATA_GATEWAY && process.env.PINATA_GATEWAY !== 'undefined') ? process.env.PINATA_GATEWAY : 'ipfs.io';
  const finalUrl = `https://${gateway}/ipfs/${data.IpfsHash}`;
  console.log('[IPFS] Generated JSON URL:', finalUrl, 'using gateway:', gateway);
  return finalUrl;
}
...
export async function uploadToPinata(
  ...
  const imageUri = await uploadFileToPinata(fileBuffer, `image-${Date.now()}.png`);
  ...
  const metadataUri = await uploadJsonToPinata(metadataObject);
